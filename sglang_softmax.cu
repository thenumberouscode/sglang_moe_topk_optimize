...
template <typename T, int TPB>
__launch_bounds__(TPB) __global__ void moeOnlineSoftmax(
    const T* input,
    const bool* finished,
    float* output,
    const int num_cols,
  ...
  for (int ii = threadIdx.x; ii < num_cols; ii += TPB) {
    ...
    thread_sum = thread_sum * expf(old_max - new_max) + expf(val - new_max);
    thread_max = new_max;
    old_max = thread_max;
  }

  const float maxElem = BlockReduce(tmpStorage).Reduce(thread_max, MaxReduceOp());
  const auto Z = BlockReduce(tmpStorage).Sum(thread_sum);
  if (threadIdx.x == 0) {
    float_max = maxElem;
    normalizing_factor = 1.f / Z;
  }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("topk_softmax", &topk_softmax, "MOE TopK Softmax");
}
